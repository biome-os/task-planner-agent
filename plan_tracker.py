...
class PlanTracker:
    ...
    def validate(
        ...
        # Detect structured execution failures that use output_data fields instead of "error"
        # (e.g. code-execution-agent returns exit_code/stderr/stdout on non-zero exit)
        if output.get("exit_code") not in (None, 0):
            parts: list[str] = [f"exit_code: {output['exit_code']}"]
            if output.get("stderr"):
                parts.append(f"stderr: {output['stderr']}")
            if output.get("stdout"):
                parts.append(f"stdout: {output['stdout']}")
            return "local_replan", "Emergent step failed: " + "; ".join(parts)
